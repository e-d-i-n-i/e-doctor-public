...
import { fetchCompletedLaboratoryTestRequests } from "@/api/fetch-laboratory-requests"; // Adjust the import path

export function DataTable() {
  const [data, setData] = useState<LaboratoryTestRequest[]>([]);
  ...
  const fetchData = async (isManualRefresh: boolean = false) => {
    try {
      const requests = await fetchCompletedLaboratoryTestRequests();
      setData(requests);
      ...
      <div className="flex flex-col space-y-4 sm:space-y-0 sm:flex-row sm:flex-wrap items-start sm:items-center py-4 gap-4">
        <div className="relative w-full sm:w-auto">
          ...
          <Input
            placeholder="Search..."
            value={
              (table.getColumn("test_type")?.getFilterValue() as string) ?? ""
            }
            onChange={(event) =>
