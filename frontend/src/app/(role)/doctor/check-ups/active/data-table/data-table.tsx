...
import { fetchActivePatientContextByDoctorUsername } from "@/api/fetch-patients";
import { getUsername } from "@/hooks/auth";

export function ActivePatientDataTable() {
  const [data, setData] = useState<PatientContext[]>([]);
  ...
  const fetchData = async (isManualRefresh: boolean = false) => {
    try {
      const username = getUsername(); // Get the username of the currently logged-in doctor
      if (!username) {
        throw new Error("No username found. Please log in.");
      }

      const activePatients = await fetchActivePatientContextByDoctorUsername(
        username
      );
      setData(activePatients);
      setLastRefreshed(new Date());

      if (isManualRefresh) {
        toast.success("Active patient data refreshed successfully!");
      }
    } catch (error) {
      console.error("Failed to fetch active patients:", error);

      if (isManualRefresh) {
        toast.error("Failed to refresh active patient data. Please try again.");
      }
    }
  };
