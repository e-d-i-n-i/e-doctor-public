...
import { fetchCompletedPatientContext } from "@/api/fetch-patients";

export function CompletedPatientDataTable() {
  ...
  const fetchData = async (isManualRefresh: boolean = false) => {
    try {
      const completedPatients = await fetchCompletedPatientContext(); // Fetch only completed patients
      setData(completedPatients);
      setLastRefreshed(new Date());

      if (isManualRefresh) {
        toast.success("Completed patient data refreshed successfully!");
      }
    } catch (error) {
      ...
      if (isManualRefresh) {
        toast.error(
          "Failed to refresh completed patient data. Please try again."
        );
      }
    }
  };
