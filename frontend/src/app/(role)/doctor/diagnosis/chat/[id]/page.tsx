...
import MessageInput from "@/components/chat/MessageInput_for_doc";
...
const ChatBox: React.FC<ChatBoxProps> = ({ params }) => {
  ...
  const loadInitialChatHistory = async (threadId: string) => {
    try {
      ...
      const response = await fetch(
        `${FLASK_BACKEND_URL}/load-doc-chat/${threadId}?limit=10`
      );
      if (response.ok) {
  ...
  const sendMessageToBackend = async (message: string) => {
    try {
      const response = await fetch(`${FLASK_BACKEND_URL}/doc-chat`, {
        method: "POST",
