...
import { fetchPendingFollowUpQuestionsByUsername } from "@/api/fetch-followUp-questions"; // Adjust the import path
import { getUsername } from "@/hooks/auth";

export function DataTable() {
  const [data, setData] = useState<FollowUpQuestion[]>([]);
  ...
  const fetchData = async (isManualRefresh: boolean = false) => {
    try {
      const username = getUsername(); // Replace with the actual username
      const questions = await fetchPendingFollowUpQuestionsByUsername(username);
      setData(questions);
