...
const formSchema = z.object({
  ...
  new_nationality: z
    .string()
    .min(1, "Nationality must be at least 1 character long"),
  current_password: z
    .string()
    .min(8, "Password must be at least 8 characters long"),
});

export default function EditNationalityForm({
  userid,
  setIsOpen,
}: {
  userid: string;
    ...
    <Form {...form}>
      ...
      >
        ...
        <FormField
          ...
          render={({ field }) => (
            <FormItem>
              <FormLabel>New Nationality</FormLabel>
              <FormControl>
                <Input
                  type="text"
...
export async function updateNationality(
  ...
  const response = await fetch(
    process.env.NEXT_PUBLIC_CHANGE_NATIONALITY_PATIENT_USER_ENDPOINT!,
    {
      method: "POST",
      headers: {
  ...
  if (!response.ok) {
    ...
    throw new Error(errorDetails.error || "Failed to update nationality.");
  }
