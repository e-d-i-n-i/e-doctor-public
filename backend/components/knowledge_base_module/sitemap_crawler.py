...
async def process_sitemap(sitemap_url: str, source: str):
...
def gradio_interface(sitemap_url: str, source: str):
    """Gradio interface to handle user input."""
    asyncio.run(process_sitemap(sitemap_url, source))
    return f"Processing completed for sitemap: {sitemap_url}"

# Gradio UI
with gr.Blocks() as demo:
    gr.Markdown("# Sitemap Processor")
    with gr.Row():
        sitemap_url_input = gr.Textbox(label="Enter Sitemap URL", placeholder="https://example.com/sitemap.xml")
    ...
    submit_button.click(
        gradio_interface,
        inputs=[sitemap_url_input, source_input],
        outputs=output
    )
