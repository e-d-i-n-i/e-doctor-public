...
async def process_websites(urls: List[str], source: str):
    """Process a list of website URLs."""
    for url in urls:
        await crawl_website(url, source)

def gradio_interface(urls: str, source: str):
    """Gradio interface to handle user input."""
    # Split the input string into a list of URLs
    url_list = [url.strip() for url in urls.split(",")]
    asyncio.run(process_websites(url_list, source))
    return f"Processing completed for {len(url_list)} URLs"

# Gradio UI
with gr.Blocks() as demo:
    gr.Markdown("# Website List Processor")
    with gr.Row():
        urls_input = gr.Textbox(label="Enter URLs (comma-separated)", placeholder="https://example.com, https://example.org")
        source_input = gr.Textbox(label="Source (e.g., medical_docs)", placeholder="Enter source name")
    submit_button = gr.Button("Process Websites")
    output = gr.Textbox(label="Output")

    submit_button.click(
        gradio_interface,
        inputs=[urls_input, source_input],
        outputs=output
    )
